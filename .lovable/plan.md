...
- **Strategic evidence — sourced from `onboarding_v8_responses`.** The CoS profile (provisional archetype, primary depletion pattern, communication profile), declared growth goals, and the events the leader themselves flagged as important / stressful / draining (stakes, load and burden chips, free-text context), with `protection_goals` / `pressure_profile` on `profiles` as the mirrored fallback. Built as an extensible source so coach conversations, evening-reflection notes, mindset-reframe notes and Week Ahead picks can plug into the same tier later.
