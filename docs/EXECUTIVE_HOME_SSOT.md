...
holidays using existing shared-module rules. The operating model is:
**scheduled cron writes snapshots; Executive Home reads snapshots; manual
refresh is the explicit on-demand writer.**
...
| Mastery Plan (Today + Week Ahead) | `TodayThreePriorities`, `WeekAheadPriorities` | `useMasteryPlanSnapshot`, `useWeekAheadMode` | `generate-mastery-plan` + `list-week-ahead-priorities` | `mastery_plan_snapshots`, `weekly_plan_snapshots`, `jit_event_context` |

All three cards are snapshot-read-first. `daily_context_snapshot` is the shared
substrate for MRS + Brief and carries mirrored plan metadata; the canonical
day-of plan payload itself lives in `mastery_plan_snapshots`. The cards do not
recompute these outputs on the client during normal page load.
...
7. The Plan is window-aware. Morning, afternoon, and evening runs may produce
   different ready snapshots when the user's physiological state, calendar, or
   lead-event context has changed.
...
All three Executive Home cards are intended to refresh on these three windows.
The Brief and the Plan are explicitly expected to change by window when the
...
| Weekend (Sat) | `ceo-behaviour/weekend.ts` | Light-day handling still applies, but windowed refreshes remain allowed when product policy wants all 3 cards kept current. Sunday Week-Ahead **must not** leak to Saturday. |
| Weekend (Sun) | `weekend.ts` | Windowed refreshes may still run; Sunday additionally owns the Week-Ahead Plan branch (`list-week-ahead-priorities`). |
| PTO | `ceo-behaviour/pto-holiday.ts` | Recovery tone / suppressed work priorities, but the snapshot model itself remains window-capable. |
...
| Light day (no high-stakes, demand<30) | derived in `day-kind-detector` | Lower-demand scoring/copy/priority selection; does not by itself prohibit a current-window snapshot. |
...
### 8.3 Manual refresh policy

Manual refresh is a shared Executive Home snapshot writer, not an isolated
card repaint.

- If a user refreshes one card/workflow on Executive Home, the system should
  regenerate the current-window snapshot set for that user/date/window.
- After a successful write, the dependent readers should invalidate and reload
  so the other cards update automatically.
- Users should not need to manually refresh each card separately on the same
  screen.

### 8.4 Read policy

- MRS reads the current-window MRS snapshot.
- Brief reads the current-window Brief snapshot.
- Plan reads the current-window ready plan snapshot for that day.
- If a current-window plan snapshot is missing, an explicit fallback to the
  latest ready row for that day is allowed only when implemented deliberately
  and logged clearly.
- Pending or error rows must never displace an already-ready visible snapshot.

---

## 9. Centralised Cron Pipeline (Target Architecture)

### 9.1 Central orchestrator edge function
...
**Why 15-minute granularity?** Window boundaries (05:00 / 12:00 / 18:00 local) must fire within 15 min for every timezone offset. The orchestrator's per-user `resolveWindow` ensures each user is only computed on the intended local windows rather than continuously on every page view.
...
- `mastery_plan_snapshots` keyed by `(user_id, plan_date, window)`; each window may persist a different ready plan when context changed.
- `weekly_plan_snapshots` keyed by `(user_id, iso_year, iso_week)`.

### 9.4 Deprecates / replaces
- Per-card client hooks continue to **read** snapshots but no longer own default compute on cold-cache. Manual refresh remains the explicit exception and should write the same shared current-window snapshot set that cron writes.
...
8. **Executive Home is snapshot-read-first.** Cron is the default writer for MRS / Brief / Plan on morning, afternoon, and evening windows. Manual refresh is the only on-demand writer.
9. **Plan is window-aware.** It is valid for a later ready plan snapshot to replace an earlier ready plan snapshot for the same date when the later run reflects changed context. Readers must choose the intended current-window or latest-ready policy explicitly; never by accident.
10. **Auth0 `sub` claim is the user id everywhere.** RLS policies now use `(auth.jwt() ->> 'sub')`. Edge functions verify via `_shared/auth.ts :: verifyAuth0JWT`.
11. **CEO-behaviour rules live in `_shared/ceo-behaviour/*` only.** No inlining in cards. The registry contract is enforced by `registry.contract.test.ts`.
...
1. **Align** `supabase/functions/build-executive-home-cards/index.ts` as the sole Executive Home orchestrator for the 3-window snapshot model.
...
6. **Remove** any per-card cold-start triggers from the client hooks once snapshot-read-first is fully live; keep the on-demand manual refresh path.
7. **Add** integration test that simulates one full day (3 windows) and asserts: one `daily_context_snapshot` per window, one `brief_snapshots` per window, and one `mastery_plan_snapshots` per window on a regular weekday; `weekly_plan_snapshots` written on Sunday according to the week-ahead branch.
8. **Add** integration coverage proving that a manual refresh from one Executive Home surface invalidates and refreshes the other two cards' readers automatically.
