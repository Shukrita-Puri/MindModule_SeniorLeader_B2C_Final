...
5. **Weekend events are dropped.** The engine buckets Mon–Fri only and discards Saturday and Sunday events before any maths. The UI now renders a full Mon–Sun week, but the two weekend columns stay empty until the engine's bucket set is widened. This matters directly for Israel and Gulf working weeks — and for this account, where two of the largest observed deltas fall on a Saturday and a Sunday.
