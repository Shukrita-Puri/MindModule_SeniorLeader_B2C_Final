...
**Update addendum (June 3, 2026 — slot-label extraction wave):**

- **F-09 — ✅ Resolved.** `composeStateLabel()` lifted out of `generate-mastery-plan/index.ts` into `supabase/functions/_shared/events/slot-labels.ts` as `composeSlotStateLabel()`. The local function is now a thin wrapper that binds closure-local helpers (`pickAnchorEvent`, `scoreEventStakes`, `truncateTitle`, `isTravelTitle`) and forwards everything else to the shared module. Priority preserved (distinct event > calendar load > wearable deficit > tomorrow/week load). Variable-slot drop rule preserved. All six call sites (slot 1 depleted, slot 1 default, slot 2, slot 3 `2immediate-1tactical`, slot 3 default, filler, forbidden-literal rewrite) now go through the shared helper.
- **F-16 — ✅ Resolved.** `buildAnchorSnapshot()` and the shared `SlotAnchorMeta` now persist the fuller `enrichEvent` snapshot (`anchorTitle`, `anchorDemandProfile`, `anchorPhases`) on every slot — including state-anchored and filler slots — not just `eventId / categoryId / subtypeId / scenarioId / leadTimeMin`. Slot consumers no longer have to re-classify from partial metadata. `HorizonModule` extended with the matching optional fields.

**Update addendum (June 3, 2026 — F-14 / F-11 / F-05 / F-10 / F-17 wave):**

- **F-14 — ✅ Resolved.** `selectLeadEvent` now consumes `event-subtypes.jitLeadTimeMinutes` via an internal `jitLeadTimeBoost` helper. Events whose start falls inside the prep horizon get +20 stakes; within 2× the horizon get +10; just-passed events (≤60 min) get +5. The boost shifts which event the Brief mentions toward the one that is actually actionable now rather than the most distant high-stakes meeting. Optional `opts.nowMs` is plumbed for deterministic testing.
- **F-11 — ✅ Resolved.** Added `buildWhyNowLine()` to `_shared/plan/action-frame.ts`. Nudges now appends a `- Why now: …` line, derived from the same `(category, phase)` pair as `buildActionFrame`, to all four event-based JIT prompts (`nudge_one_jit`, `nudge_two_jit`, `nudge_two_recalibrate`, `nudge_two_reserves`). Plan and Nudges now share one deterministic "why now" contract; no extra LLM round-trip required.
- **F-05 — ✅ Resolved (telemetry phase).** `classifyPatternBucket()` now emits a `console.warn` whenever the legacy `EVENT_TYPE_KEYWORDS` fallback fires. Canonical-subtype resolution is unchanged. Once production logs show zero fallback hits across a full causality window, the keyword table and `signal_summary` legacy labels can be retired in a follow-up.
- **F-10 — ✅ Resolved.** Confirmed `buildRecommendedAction` in `generate-mastery-plan/index.ts` is a pure passthrough to shared `buildRecommendedActionCopy` (no local strings). All non-anchored generic slots already go through the shared copy helper — no drift surface remains.
- **F-17 — ✅ Resolved.** Plan response now stamps `prompt_version: BRIEF_PROMPT_VERSION` on every successful JSON payload. Nudges stamps `prompt_version` on every shipped notification payload AND on the function-level summary response. Brief, Plan, and Nudges artefacts now share a single cross-feature trace key.

| # | Status | Resolution notes |
|---|--------|------------------|
| F-01 | ✅ Resolved | Wave 1: every deterministic Brief prompt section is now built by a pure helper in `_shared/brief/prompt-blocks.ts` (`buildContextBlock`, `buildReadinessBlock`, `buildWearableBlock`, `buildCalendarTodayBlock`, `buildTomorrowBlock`, `buildWeekAheadBlock`, `buildPatternsBlock`, `buildOnboardingBlock`, `buildKeySignalsBlock`, `buildGlobalLoadBlock`, `buildStrategicContextHead`, `buildTriangulationBlock`, `buildWindowContextBlock`). The orchestrator builds a typed inputs bag and calls each builder; behaviour is byte-for-byte equivalent to the prior inline assembly. Event-coaching blocks (`classifyEvent` + `phaseForEvent`) remain as shipped previously. |
...
| F-12 | ✅ Resolved | Wave 1: the 200-line inline `userPrompt += …` accumulator is gone. All block ordering, omission rules, and field formatting (signed deviations, time strings, null fallbacks) live in `_shared/brief/prompt-blocks.ts`; the orchestrator now only assembles typed inputs. |
...
The remainder of the findings (F-05, F-10, F-11, F-14, F-17) remain open. F-09 / F-16 (Plan slot labels + filler enrichment) shipped earlier; F-01 / F-12 (Brief block-builders) shipped in Wave 1. The Brief follow-up is now F-14 (event-selection JIT lead-time weighting).
...
|---|----------|---------|------------------------|------------------------|
| F-01 | S1 | Brief | Resolved June 3 (Wave 1): every deterministic Brief prompt section is now built by a pure helper in `_shared/brief/prompt-blocks.ts`; the orchestrator only assembles typed inputs. Event-coaching blocks remain as shipped previously. | Shipped. Keep new sections in the shared module; the orchestrator should not grow new inline `userPrompt += ...` chains. |
...
| F-05 | S1 | Nudges | ✅ Resolved (telemetry phase). `classifyPatternBucket()` emits a `console.warn` whenever the legacy keyword fallback fires. Once production logs show zero fallback hits, the keyword table can be retired in a follow-up. | Monitor warn count → drop `EVENT_TYPE_KEYWORDS` table + migrate `signal_summary` bucket labels. |
...
| F-09 | S2 | Plan | Resolved June 3 (addendum): `composeStateLabel()` lifted to `_shared/events/slot-labels.ts` as `composeSlotStateLabel()`; the local function is now a thin wrapper. All six slot call sites delegate to the shared helper. | Shipped. Keep future state-anchor logic in the shared module. |
| F-10 | S2 | Plan | ✅ Resolved. The local `buildRecommendedAction` helper is now a pure passthrough to shared `buildRecommendedActionCopy`; no local copy strings remain in Plan for non-anchored generic slots. | Shipped. |
| F-11 | S2 | Nudges | ✅ Resolved. `buildWhyNowLine()` added to `_shared/plan/action-frame.ts`; all four event-based JIT nudges (`nudge_one_jit`, `nudge_two_jit`, `nudge_two_recalibrate`, `nudge_two_reserves`) prepend a deterministic `- Why now: …` line derived from the same `(category, phase)` pair the Plan uses. | Shipped. |
| F-12 | S2 | Brief | Resolved June 3 (Wave 1): the 200-line inline `userPrompt += …` accumulator is replaced by typed-input calls to `_shared/brief/prompt-blocks.ts`. Block ordering, omission rules, and field formatting all live in the shared module. The async STRATEGIC CONTEXT v2 hydration stays inline (DB read). | Shipped. Future Brief sections must land as new builder exports rather than inline assembly. |
...
| F-14 | S3 | Brief | ✅ Resolved. `selectLeadEvent` now consumes `event-subtypes.jitLeadTimeMinutes` via `jitLeadTimeBoost` (+20 inside horizon, +10 within 2× horizon, +5 for just-passed). Optional `opts.nowMs` plumbed for testing. | Shipped. |
...
| F-16 | S3 | Plan | Resolved June 3 (addendum): every slot — anchored, state-anchored, and filler — now persists the fuller `enrichEvent` snapshot via `SlotAnchorMeta` (`anchorTitle`, `anchorDemandProfile`, `anchorPhases`) on top of the existing anchor ids. `HorizonModule` extended; `buildAnchorSnapshot()` returns the richer shape. | Shipped. Downstream surfaces can read enriched anchor metadata without re-running `classifyEvent`. |
| F-17 | S3 | All three | ✅ Resolved. Plan response payload stamps `prompt_version: BRIEF_PROMPT_VERSION`; Nudges stamps it on shipped notification payloads and on the function-level summary response. Brief was already stamping. | Shipped. |

**Top open risks (read first):**
1. Pattern-store compatibility still depends on historical bucket labels even though classification now resolves from canonical subtypes first (F-05).
2. Plan still carries a local copy bridge for non-anchored generic slots (F-10). State-label extraction and filler-slot enrichment have shipped (F-09, F-16).
3. Nudges now gets shared event-frame cues, but its broader prompt scaffolding and "why now" logic are still hand-authored, so copy logic can still drift from Plan (F-11).
4. Brief event selection still ignores `event-subtypes.jitLeadTime` when picking which event to mention (F-14).
5. `BRIEF_PROMPT_VERSION` is stamped by Brief only; Plan and Nudges stamp their own `architecture` field, leaving cross-feature trace ambiguity (F-17).

**Remediation waves** (dependency-ordered, no implementation here):
- **Wave 1 — Brief block-builders + event selection:** F-01 ✅ + F-12 ✅ shipped via `_shared/brief/prompt-blocks.ts`. F-14 (event-selection JIT lead-time weighting) remains.
...
- **Wave 3 — Plan bridge cleanup + observability:** F-10, F-17. F-09 + F-16 shipped (slot-labels extraction + richer filler-slot persistence). Remaining: consolidate non-anchored generic slot copy and stamp `BRIEF_PROMPT_VERSION` on Plan + Nudges artefacts.
