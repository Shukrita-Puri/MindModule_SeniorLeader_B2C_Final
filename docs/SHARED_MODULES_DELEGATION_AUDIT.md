...
The remaining open shared-module migration work (F-01, F-11, F-12, F-14) is tracked below and deferred to a follow-up wave because it touches the ~5,500-line Brief prompt assembly and the Nudges per-event framing; landing it safely needs a dedicated change set with a parked-legacy parallel path. The shared module scaffold (`buildBriefSystemPrompt`, `buildWindowContext`, behaviour snapshot, event-coaching blocks, `composeSlotStateLabel`, `buildActionFrame`) is now in place and ready to absorb the next wave.
...
3. Plan still carries a local copy bridge for non-anchored generic slots (F-10). State-label extraction and filler-slot enrichment have shipped (F-09, F-16).
