...
**Prompt-boundary note:** DB-backed STRATEGIC CONTEXT v2 hydration stays in the Brief orchestrator because it needs live database access. Everything that merely projects that context into prompt text should still be shared unless there is a hard dependency on the local runtime.
...
| F-12 | 🟡 Partial | `=== TIME ===` block renamed to canonical `=== CONTEXT: [MORNING\|AFTERNOON\|EVENING] ===` per §8. The remaining inline accumulator must be replaced by `buildSignalCoverage`; keeping the coverage builder in Brief is no longer the target state. |
...
The remainder of the findings (F-05, F-09, F-11, F-12, F-14, F-16) remain open/partial. The shared prompt + event-coaching boundary now exists; the remaining Brief work is to remove the inline coverage accumulator and move the event-selection / prompt-projection logic behind shared helpers.
...
|---|----------|---------|------------------------|------------------------|
| F-01 | S1 | Brief | Partial June 3: Brief now appends `=== TODAY EVENT COACHING ===` / `=== TOMORROW EVENT COACHING ===` blocks built from `classifyEvent` + `phaseForEvent`, so lead events are no longer title-only. The remaining gap is replacing the broader inline coverage builder with the shared signal-coverage module. | Keep the shared event-coaching blocks, then move the rest of the prompt coverage into `_shared/brief-signal-coverage.ts` so Brief only projects shared signal coverage, not reassembles it. |
...
| F-11 | S2 | Nudges | Partial June 3: event-based nudges now receive shared event-frame cues via `_shared/plan/action-frame.ts`, but the per-nudge prompt scaffolding is still largely hand-authored and does not yet use `why-llm`. | Keep event-based nudges on shared `action-frame` cues, then move the remaining scaffolding and "why now" generation behind shared helpers so Nudges stops inventing its own framing path. |
| F-12 | S2 | Brief | Partial June 3: the canonical `=== CONTEXT: [MORNING|AFTERNOON|EVENING] ===` header and `PRE_COMPUTED_USER_NOTICE` landed, but the large inline `userPrompt += …` accumulator is still in place instead of delegating to `buildSignalCoverage` (`compute-outer-readiness/index.ts:3539–3888`). | Replace the remaining accumulator with `_shared/brief-signal-coverage.ts`; block ordering and omission rules must live in one shared helper, not a local append chain. |
