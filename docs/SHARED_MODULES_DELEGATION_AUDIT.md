...
The remaining open shared-module migration work (F-11, F-14) is tracked below. **Wave 1 (Brief block-builders) shipped** — the deterministic Brief prompt sections (CONTEXT, READINESS, WEARABLE, CALENDAR TODAY, TOMORROW, WEEK AHEAD, PATTERNS, ONBOARDING, KEY SIGNALS, GLOBAL & ENVIRONMENTAL LOAD, STRATEGIC CONTEXT head, TRIANGULATION, WINDOW CONTEXT) are now built by pure helpers in `_shared/brief/prompt-blocks.ts`; the orchestrator only assembles typed inputs and calls the builders in canonical order. The async STRATEGIC CONTEXT v2 hydration (snapshot → composeDailyContext) remains inline because it requires DB access. F-11 (Nudges "why now") and F-14 (Brief event selection JIT lead-time) remain open.
