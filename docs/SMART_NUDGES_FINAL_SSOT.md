...
3. **State 1 (Early Read) basis.** Because the nudge fires *before* check-in, the data it sees is **State 1 (baseline / Early Read)** — wearable + calendar + pattern, no check-in yet. The nudge's whole job is to move the user to **State 2** (check in). This is why it is proactive, and why most CTAs are "check in."
