...
- **Fail-open hydration.** Every upstream signal (todayEvents, tomorrowEvents, lookbackEventsRaw → ptoToday / ptoTomorrow / holiday / consecutiveOffDaysBefore) defaults to a safe value when missing. A `[week-ahead-hydration]` log line is emitted for every defaulted field so a silent week is diagnosable from edge logs alone.
