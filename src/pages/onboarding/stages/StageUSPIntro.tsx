...
export default function StageUSPIntro() {
  ...
  return (
    <div className="fixed inset-0 z-40 flex flex-col bg-app-surface text-[#1a1712] overflow-hidden pb-[env(safe-area-inset-bottom,0px)]">
      ...
      <div className="relative shrink-0 h-[56vh] overflow-hidden">
        <img
          src={SLIDES[idx].image}
          alt=""
          aria-hidden
          className="absolute inset-0 w-full h-full object-cover"
          style={{ filter: "grayscale(1) contrast(1.05)", objectPosition: "center 72%" }}
        />
        <div
          className="absolute inset-x-0 bottom-0 h-[12%] pointer-events-none"
          style={{
            background:
              "linear-gradient(to top, hsl(var(--canvas-hi)) 0%, hsl(var(--canvas-hi) / 0.25) 55%, hsl(var(--canvas-hi) / 0) 100%)",
          }}
        />
      </div>
