 ...
 *
 * Confirms that Planner, Brief (`buildRuleContext` +
 * `signals.ptoTodayAllDay`), and Smart Nudges (`ptoMode` +
 * `weekAheadInputs.pto/holidayTodayAllDay`) all reach the SAME decision
...
// Fixed reference dates. `getDay()` on these Dates returns the UTC day, which
// matches Deno's default runtime timezone (UTC on edge workers).
const MON_9AM = new Date("2026-01-05T09:00:00Z"); // Monday (UTC day=1)
const SAT_9AM = new Date("2026-01-03T09:00:00Z"); // Saturday (UTC day=6)
...
interface Scenario {
  ...
  /** Brief `signals.ptoTodayAllDay` — collapses PTO + applicable HOLIDAY. */
  expectedBriefPtoTodayAllDay: boolean;
  /** Nudges `weekAheadInputs.ptoTodayAllDay` — PTO only. */
  expectedNudgesPtoTodayAllDay: boolean;
  /** Nudges `weekAheadInputs.holidayTodayAllDay` — PUBLIC_HOLIDAY only. */
  expectedNudgesHolidayTodayAllDay: boolean;
  /** Nudges `dayContext.ptoMode` — PTO OR PUBLIC_HOLIDAY. */
  expectedNudgesPtoMode: boolean;
  /** Planner `deriveStructuralDayFlags` → `hasRestSignals`. */
...
const scenarios: Scenario[] = [
  {
    ...
    expectedBriefPtoTodayAllDay: false,
    expectedNudgesPtoTodayAllDay: false,
    expectedNudgesHolidayTodayAllDay: false,
    expectedNudgesPtoMode: false,
  ...
  },
  {
    ...
    expectedBriefPtoTodayAllDay: true,
    expectedNudgesPtoTodayAllDay: false,
    expectedNudgesHolidayTodayAllDay: true,
    expectedNudgesPtoMode: true,
  ...
  },
  {
    ...
    expectedIsRestDay: true,
    expectedBriefPtoTodayAllDay: true,
    expectedNudgesPtoTodayAllDay: true,
    expectedNudgesHolidayTodayAllDay: false,
    expectedNudgesPtoMode: true,
    expectedPlannerHasRestSignals: true,
  },
  ...
  {
    name: "Saturday with 3 work meetings (work evidence overrides weekend)",
    now: SAT_9AM,
    events: [
  ...
  {
    name: "Public holiday with 3 work meetings (work evidence overrides holiday)",
    now: MON_9AM,
    events: [
      {
        title: "Bank Holiday (England & Wales)",
...
// Mirror of Brief's signal-coverage override.
function briefDerivation(a: AvailabilityResult) {
  const pto =
    a.state === "PTO" ||
    (a.state === "PUBLIC_HOLIDAY" && a.holiday.applicable);
...
for (const s of scenarios) {
  Deno.test(`SSOT parity — ${s.name}`, () => {
    ...
    assertEquals(availability.state, s.expectedState, "classifier state");
    assertEquals(availability.isRestDay, s.expectedIsRestDay, "classifier isRestDay");
    ...
    //    signals.ptoTodayAllDay must be gated on it.
    const ctx = buildRuleContext({
      ...
      events: s.events.map((e) => ({
        ...
        calendarSummary: e.calendarSummary ?? null,
      })),
      now: s.now,
      userHomeCountry: s.userHomeCountry,
      explicitPto: s.explicitPto === true,
    ...
    assertEquals(
      ...
      s.expectedBriefPtoTodayAllDay,
      "Brief ptoTodayAllDay (derivation)",
    );
    assertEquals(
      ctx.signals.ptoTodayAllDay === true,
      s.expectedBriefPtoTodayAllDay,
      "Brief signals.ptoTodayAllDay (final)",
    );

    // 4. Smart Nudges — dayContext.ptoMode + weekAheadInputs derivation.
    const nudges = nudgesDerivation(availability);
    assertEquals(nudges.ptoMode, s.expectedNudgesPtoMode, "Nudges ptoMode");
    assertEquals(
      nudges.ptoTodayAllDay,
      s.expectedNudgesPtoTodayAllDay,
      "Nudges weekAheadInputs.ptoTodayAllDay",
    );
    assertEquals(
      nudges.holidayTodayAllDay,
      s.expectedNudgesHolidayTodayAllDay,
