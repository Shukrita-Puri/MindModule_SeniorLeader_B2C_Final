...
function computeDurationMinutes(raw: any): number | null {
  const start = raw?.start_time ?? raw?.startTime ?? raw?.start ?? raw?.event?.start_time;
...
function computeTravelArc(subtypeId: string | null, durationMin: number | null): TravelArc | null {
  if (!subtypeId) return null;
  const isFlight = subtypeId === 'trv.flight' || subtypeId === 'trv.long_haul' || subtypeId === 'trv.travel_day';
  if (!isFlight) return null;
  // long_haul + travel_day always route to full arc even without duration
  if (subtypeId === 'trv.long_haul' || subtypeId === 'trv.travel_day') return 'pre-during-post';
 ...
 * Example: `str.deep_work` → `deep_work`, `trv.long_haul` → `long_haul`.
 * Consumed by Insights (secondary line), Week-Ahead persistence and Nudges.
 */
export function subcategoryFromSubtypeId(id: string | null | undefined): string | null {
  if (!id) return null;
