...
Deno.test("v2 rows: every new subtype resolves to a known category (A–H)", () => {
  const newIds = [
    ...
    'str.learning', 'str.community', 'str.review', 'str.compliance',
    'trv.accommodation', 'trv.travel_day',
    'rhy.holiday',
    'rhy.wellness_fitness', 'rhy.wellness_self_care',
    'rhy.wellness_health_check', 'rhy.wellness_medical',
    'rhy.social', 'rhy.family', 'rhy.recreation',
  ];
  for (const id of newIds) {
    const row = EVENT_TYPES.find((e) => e.id === id);
    assert(row, `${id} missing from EVENT_TYPES`);
    assert(['A','B','C','D','E','F','G','H'].includes(row!.categoryId), `${id} bad categoryId`);
  }
});
...
Deno.test("spec: 'Pitch to L\\'Oreal' → B (pitch_competitive), not fundraising", () => {
  const enriched = enrichEvent({ title: "Pitch to L'Oreal — RFP presentation" });
...
Deno.test("spec: 'Q2 results to leadership' → C (stakeholder_comm)", () => {
  const enriched = enrichEvent({ title: 'Q2 results to leadership' });
  assertEquals(enriched.categoryId, 'C');
  assertEquals(enriched.subcategory, 'stakeholder_comm');
});

// ── Spec Part 8 — Plan Logic Arcs ──────────────────────────────────

function isoRange(startISO: string, durationMin: number): { start_time: string; end_time: string } {
...
Deno.test("arc: 12h flight → pre-during-post (full arc)", () => {
  const enriched = enrichEvent({ title: 'Flight LHR → SFO', ...isoRange('2026-08-01T10:00:00Z', 720) });
  assertEquals(enriched.subcategory, 'flight');
  assertEquals(enriched.travelArc, 'pre-during-post');
});

Deno.test("arc: 2h flight → pre-post", () => {
  const enriched = enrichEvent({ title: 'Flight LHR → BCN', ...isoRange('2026-08-01T10:00:00Z', 120) });
  assertEquals(enriched.subcategory, 'flight');
...
Deno.test("arc: long-haul explicit → pre-during-post regardless of duration", () => {
  ...
  assertEquals(enriched.subcategory, 'long_haul');
  assertEquals(enriched.travelArc, 'pre-during-post');
});
...
Deno.test("arc: passive learning webinar → travelArc null (Plan will skip arc)", () => {
  const enriched = enrichEvent({ title: 'Webinar — Assumptions of resilience' });
  assertEquals(enriched.subcategory, 'learning');
  assertEquals(enriched.travelArc, null);
});
...
Deno.test("subcategoryFromSubtypeId derives correctly", () => {
  ...
  assertEquals(subcategoryFromSubtypeId('trv.long_haul'), 'long_haul');
