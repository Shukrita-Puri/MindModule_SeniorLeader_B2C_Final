...
Deno.test("USES the filtering selectJitCandidates in week-ahead", () => {
  assertEquals(/from\s+["'][^"']*select-jit/.test(SRC), true);
  assertEquals(/selectJitCandidates\s*\(/.test(SRC), true);
});
