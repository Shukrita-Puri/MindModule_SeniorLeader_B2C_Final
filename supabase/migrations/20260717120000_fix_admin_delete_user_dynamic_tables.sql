-- Admin delete hardening:
-- The original RPC used a fixed table list, which broke whenever a new
-- user-owned table was added after the migration. Keep the explicit handling
-- for non-user_id relationships, then delete every current public table with a
-- user_id column before deleting the profile.
...
BEGIN
  ...
  -- Dialogue subordinate tables are linked by session_id, not user_id.
  ...
  -- Referral conversions point to users through two non-user_id columns.
